...
#    peer chaincode query -C $CHANNEL_NAME -n fabcar -c '{"Args":["query","a"]}' >&log.txt
    peer chaincode query -C $CHANNEL_NAME -n fabcar -c '{"Args":["queryCar", "Toyota"]}' >&log.txt
    res=$?
    set +x
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -xmake sure the chaincode
    peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n fabcar $PEER_CONN_PARMS -c '{"Args":["createCar", "CAR12", "Honda", "Accord", "Black", "Tom"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n fabcar $PEER_CONN_PARMS -c '{"Args":["createCar", "CAR12", "Honda", "Accord", "Black", "Tom"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
