...
# upgrading chaincode on the network previously setup in the
# BYFN tutorial.
#
# Copyright Nguyen Van Tien, 2019
...
function printHelp () {
  echo "Usage: "
  echo "  upgrade.sh upgrade [-c <channel name>] [-t <timeout>] [-d <delay>] [-e <chaincode version>]"
  echo "  upgrade.sh -h|--help (print this message)"
  echo "    <mode> - 'upgrade'"
  ...
  echo "    -e <version> - chaincode version"
  echo "    -v - verbose mode"
  ...
  echo
  echo "	upgrade.sh upgrade -c mychannel -e version"
  echo
}
...
#  fi
  echo
  echo "###############################################################"
  echo "##### Upgrade chaincode on the network #####"
  echo "###############################################################"
  docker exec Org3cli ./scripts/upgradeChaincodeOrg3.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE $VERSION
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to upgrade chaincode on network"
    exit 1
  fi

  docker exec cli ./scripts/upgradeChaincode.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE $VERSION
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to upgrade chaincode on network"
    exit 1
  fi
...
while getopts "h?c:t:d:f:e:i:v" opt; do
  case "$opt" in
    h|\?)
      printHelp
      exit 0
